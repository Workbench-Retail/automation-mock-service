...
import { SessionData as MockSessionData } from "./RET15/session-types";
import { createMockResponse } from "./RET15/version-factory";

export { MockSessionData };

const actionConfig = yaml.load(
  readFileSync(path.join(__dirname, "./RET15/factory.yaml"), "utf8")
) as any;

export const defaultSessionData = () => yaml.load(
  readFileSync(path.join(__dirname, "./RET15/session-data.yaml"), "utf8")
...
export function getSaveDataContent(version: string, action: string) {
  let actionFolderPath = path.resolve(
    __dirname,
    `./RET15/APPLIANCES/${version}/${action}`
  );
