...
import { SessionData as MockSessionData } from "./RET13/session-types";
import { createMockResponse } from "./RET13/version-factory";

export { MockSessionData };

const actionConfig = yaml.load(
  readFileSync(path.join(__dirname, "./RET13/factory.yaml"), "utf8")
) as any;

export const defaultSessionData = () => yaml.load(
  readFileSync(path.join(__dirname, "./RET13/session-data.yaml"), "utf8")
