...
export async function on_select_offers_generator(
  existingPayload: any,
  ...
  existingPayload.message.order.quote.breakup.forEach((element: any) => {
    let itemId = element["@ondc/org/item_id"];
    selectedItemsObj.forEach((item) => {
      if (item.id === itemId) {
        element["@ondc/org/item_quantity"] = {
          count: item.quantity.count,
        };
        element.price.value = String(element.item.price.value * element["@ondc/org/item_quantity"].count);
      }
    });
  });
  let totalPrice = 0;
  existingPayload.message.order.quote.breakup.forEach((element: any) => {
   totalPrice += parseFloat(element.price.value);
  });
