...
export class TransactionCacheService {
	tryLoadTransaction = async (transactionId: string, subscriberUrl: string) => {
		...
		if (await this.checkIfTransactionExists(key)) {
			logInfo({
				message: "Exiting tryLoadTransaction Function. Transaction found in cache. Calling loadTransactionThatExists",
				meta: { transactionId, subscriberUrl },
				transaction_id: transactionId,
			});
		...
		logInfo({
			message: "Exiting tryLoadTransaction Function. Transaction not found in cache",
			meta: { transactionId, subscriberUrl },
			transaction_id: transactionId,
		});
	...
	loadTransactionThatExists = async (transSubKey: string) => {
		logInfo({
			message: "Entering loadTransactionThatExists Function. Loading transaction",
			meta: { transSubKey },
		});
		...
		if (!rawData) {
			...
			logInfo({
				message: "Exiting loadTransactionThatExists Function. Transaction not found",
				meta: { transSubKey },
			});
			throw new Error(`Transaction with id ${transSubKey} not found`);
		}
		logInfo({
			message: "Exiting loadTransactionThatExists Function. Transaction data loaded successfully",
			meta: { transSubKey },
		});
...
export class SessionCacheService {
	checkIfSessionExists = async (sessionId?: string) => {
		logInfo({
			message: "Entering checkIfSessionExists Function. Checking if session exists",
			meta: { sessionId },
		});
		if (!sessionId) {
	...
	loadSessionThatExists = async (sessionId: string) => {
		...
		logInfo({
			message: "Exiting loadSessionThatExists Function. Session data loaded successfully",
			meta: { sessionId },
		});
